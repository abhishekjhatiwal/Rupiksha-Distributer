...
class SendOtpUseCase @Inject constructor(private val repository: OtpRepository) {
    suspend operator fun invoke(phoneNumber: String): Resource<String> =
        repository.sendOtp(phoneNumber)
}

class VerifyOtpUseCase @Inject constructor(private val repository: OtpRepository) {
